...
describe("normalizePublicProxyHost", () => {
  ...
  it("rejects malformed labels and over-long hosts", () => {
    ...
    expect(
      normalizePublicProxyHost("a." + "b".repeat(300) + ".com"),
    ).toBeNull(); // > 253
  });
  ...
  it("accepts valid public hosts, normalising case / www. / trailing dot", () => {
    ...
    expect(normalizePublicProxyHost("API.Metagraph.SH")).toBe(
      "api.metagraph.sh",
    );
    ...
    expect(normalizePublicProxyHost("a.b.c.example.org")).toBe(
      "a.b.c.example.org",
    );
  });
});
